"""
Using Calibre's `fetch-ebook-metadata <https://manual.calibre-ebook.com/generated/en/fetch-ebook-metadata.html>`_ tool,
...
def fetch_metadata_map(title=None, author=None, isbn=None):
    ...
    At least one of title, author, or ISBN is required; it is suggested to either provide ISBN or both title and author. 
    In the case of multiple books matching a particular set of identifiers, the first one found will be returned
...
class fetched_metadata_and_cover:
    ...
    def __enter__(self):
        metadata = fetch_cover(self.title, self.author, self.isbn, self.cover_filename)
