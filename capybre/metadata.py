"""
Using Calibre's `ebook-meta <https://manual.calibre-ebook.com/generated/en/ebook-meta.html>`_ tool, extracts metadata and cover images from ebook files
"""
...
class Metadata():
    ...
    The title field is guaranteed to exist, but everything else may just be an empty string

    Args:
        title (str): Title
        author (str): Author
        author_sort (str): String by which the author should be sorted
        tags (List[str]): List of tags, inluding possible rating, series, and publisher tags
        isbn (str): ISBN
    ...
    def __init__(self, title='', author='', author_sort='', tags=[], isbn='', description='', publication_date=None):
        self.title       = title
        self.author      = author
...
CALIBRE_ISBN_RN   = re.compile('isbn:(.+)')
...
def extract_metadata_map(input_file: str):
    ...
    Args:
        input_file (str): path to the input file
    Returns:
        Dict mapping between metadata keys and values as directly output from the ebook-meta call
    """
...
def extract_raw_metadata_map(raw_metadata: List[str]):
    ...
    Args:
        raw_metadata (List[str]): String representation of the lines produced by Calibre (also produced by a call to ``fetch-ebook-metadata``)
    Returns:
        Dict mapping between metadata keys and values as directly output from the ebook-meta call
    """
    metadata_lines = []
    for line in raw_metadata:
        # VERY brittle, but it looks like this is the only way to read the output in case 
        # there is a newline in the value (as seems to happen often in the description)
...
def extract_cover(input_file: str, output_file: str='cover.jpg', suppress_output=True):
...
class extracted_cover_fileobj:
    """Extracts the cover image and temporarily presents it as a fileobj context object, which it then cleans
...
def get_isbn(mmap):
    ...
    if IDENTIFIERS in mmap:
        ...
        for id in identifiers:
            match = CALIBRE_ISBN_RN.match(id)
            if match:
                return match[1]
    return ''
